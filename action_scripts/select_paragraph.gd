...
func _run_action() -> void:
	for caret in Global.get_editor().get_caret_count():
		...
		while (
				paragraph.x > 0
				and not Global.get_editor().get_line(paragraph.x - 1).strip_edges() == ""
			):
			paragraph.x -= 1
		while (
				paragraph.y < Global.get_editor().get_line_count() - 1
				and not Global.get_editor().get_line(paragraph.y + 1).strip_edges() == ""
			):
			paragraph.y += 1
